...
# Build the project's OWN test suite (unit + integration + bench targets) with the
# project's NORMAL flags (no sanitizer, no --cfg fuzzing) so mayhem/test.sh only RUNS
# the prebuilt binaries. A SEPARATE target dir keeps the fuzz RUSTFLAGS out of this
# clean build. The manifest lists every test executable for test.sh.
echo "=== building upstream test suite (cargo test --no-run, normal flags) ==="
TEST_TARGET_DIR="$SRC/mayhem/test-target"
MANIFEST="$SRC/mayhem/test-bins.txt"
env -u RUSTFLAGS CARGO_TARGET_DIR="$TEST_TARGET_DIR" \
  cargo test --no-run --message-format=json \
  | grep -o '"executable":"[^"]*"' | cut -d'"' -f4 | grep -v '/examples/' | sort -u > "$MANIFEST"
[ -s "$MANIFEST" ] || { echo "ERROR: cargo test --no-run produced no test executables" >&2; exit 1; }
echo "test executables:"; cat "$MANIFEST"
